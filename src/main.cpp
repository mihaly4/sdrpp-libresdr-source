...
SDRPP_MOD_INFO{
    /* Name:            */ "libresdr_source",
    /* Description:     */ "LibreSDR source module for SDR++",
    /* Author:          */ "Ryzerth",
    /* Version:         */ 0, 2, 0,
    /* Max instances    */ 1
};
...
class LibreSDRSourceModule : public ModuleManager::Instance {
public:
    LibreSDRSourceModule(std::string name) {
        this->name = name;
        ...
        sigpath::sourceManager.registerSource("LibreSDR", &handler);
    }

    ~LibreSDRSourceModule() {
        stop(this);
        sigpath::sourceManager.unregisterSource("LibreSDR");
    }
...
private:
    ...
    void refresh() {
        ...
        for (ssize_t i = 0; i < count; i++) {
            ...
            // If the device is not a libresdr, don't include it
            if (desc.find("LibreSDR") == std::string::npos) {
    ...
    static void menuSelected(void* ctx) {
        LibreSDRSourceModule* _this = (LibreSDRSourceModule*)ctx;
        core::setInputSampleRate(_this->samplerate);
        flog::info("LibreSDRSourceModule '{0}': Menu Select!", _this->name);
    }

    static void menuDeselected(void* ctx) {
        LibreSDRSourceModule* _this = (LibreSDRSourceModule*)ctx;
        flog::info("LibreSDRSourceModule '{0}': Menu Deselect!", _this->name);
    }
    ...
    static void start(void* ctx) {
        ...
        if (_this->ctx == NULL) {
            flog::error("Could not open libre ({})", _this->uri);
            return;
        }
        ...
        flog::info("LibreSDRSourceModule '{0}': Start!", _this->name);
    }
    ...
    static void stop(void* ctx) {
        ...
        flog::info("LibreSDRSourceModule '{0}': Stop!", _this->name);
    }
    ...
    static void tune(double freq, void* ctx) {
        ...
        flog::info("LibreSDRSourceModule '{0}': Tune: {1}!", _this->name, freq);
    }
    ...
    static void menuHandler(void* ctx) {
        ...
        SmGui::FillWidth();
        SmGui::ForceSync();
        if (SmGui::Combo("##libresdr_dev_sel", &_this->devId, _this->devices.txt)) {
        ...
        if (SmGui::Combo(CONCAT("##_libre_sr_", _this->name), &_this->srId, _this->samplerates.txt)) {
        ...
        SmGui::FillWidth();
        SmGui::ForceSync();
        if (SmGui::Button(CONCAT("Refresh##_libre_refr_", _this->name))) {
            _this->refresh();
        ...
        SmGui::FillWidth();
        if (SmGui::Combo(CONCAT("##_libre_bw_", _this->name), &_this->bwId, _this->bandwidths.txt)) {
        ...
        SmGui::FillWidth();
        SmGui::ForceSync();
        if (SmGui::Combo(CONCAT("##_libre_gainmode_select_", _this->name), &_this->gmId, _this->gainModes.txt)) {
            if (_this->running) {
        ...
        SmGui::FillWidth();
        if (SmGui::SliderFloatWithSteps(CONCAT("##_libre_gain__", _this->name), &_this->gain, -1.0f, 73.0f, 1.0f, SmGui::FMT_STR_FLOAT_DB_NO_DECIMAL)) {
            if (_this->running) {
    ...
    static void worker(void* ctx) {
        LibreSDRSourceModule* _this = (LibreSDRSourceModule*)ctx;
        int blockSize = _this->samplerate / 100.0f;
...
MOD_EXPORT void _INIT_() {
    ...
    defConf["device"] = "";
    defConf["devices"] = {};
    config.setPath(core::args["root"].s() + "/libresdr_source_config.json");
    config.load(defConf);
...
MOD_EXPORT ModuleManager::Instance* _CREATE_INSTANCE_(std::string name) {
    return new LibreSDRSourceModule(name);
}

MOD_EXPORT void _DELETE_INSTANCE_(ModuleManager::Instance* instance) {
    delete (LibreSDRSourceModule*)instance;
}
